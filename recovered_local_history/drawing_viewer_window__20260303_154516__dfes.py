...
class DrawingViewerWindow(QMainWindow):
    ...
    def __init__(
        ...
        *,
        ...
        # Use the same per-user/per-machine settings namespace as the rest of the app.
    ...
    def _build_toolbar(self) -> None:
        ...
        try:
            ...
            # Defaults per request: Orange, Yellow, No color (left-to-right).
            # We also migrate older installs that had all three defaults set to white.
            def _get_bf_swatch(key: str, fallback: str) -> str:
                try:
                    has_key = bool(getattr(self._settings, "contains")(key))
                except Exception:
                    has_key = False
                try:
                    raw = str(self._settings.value(key, "", type=str) or "").strip()
                except Exception:
                    raw = ""

                v = raw.strip().lstrip("#").upper()
                v = "".join(ch for ch in v if ch in "0123456789ABCDEF")
                if len(v) != 6:
                    v = ""

                # If missing, seed with fallback.
                if not has_key or not v:
                    try:
                        self._settings.setValue(key, str(fallback or ""))
                    except Exception:
                        pass
                    return str(fallback or "")

                # Migrate old default of white.
                if v == "FFFFFF" and (fallback or "").upper() != "FFFFFF":
                    try:
                        self._settings.setValue(key, str(fallback or ""))
                    except Exception:
                        pass
            ...
            bf1 = _get_bf_swatch("pdf_viewer/bubble_backfill_swatch1_rgb", "FFA500")  # orange
            bf2 = _get_bf_swatch("pdf_viewer/bubble_backfill_swatch2_rgb", "FFFF00")  # yellow
            # Represent "No color" as empty string/None.
            bf3_raw = _get_bf_swatch("pdf_viewer/bubble_backfill_swatch3_rgb", "")
            bf3 = str(bf3_raw or "").strip()
            ...
            bf_btn3.setToolTip("Backfill color 3 (No color; right-click to change)")
